		...
		<ChatBoxScroll style={{scrollbarWidth:'none'}}>
			{messages && messages.map( (m, i) => (
					<div style={{display: "flex"}} key={m._id}>
						...
						<span style={{backgroundColor: `${m.sender._id === user._id ? "#BEE3F8" : "#B9F5D0"}`, marginLeft: isSameSenderMargin( messages, m, i, user._id ), marginTop: isSameUser( messages, m, i, user._id ) ? 3 : 10, borderRadius: "20px 20px", background: "#BEE3F8", padding: "5px 15px",}}>
							{m.content}
